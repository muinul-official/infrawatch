...
public class MainActivity extends AppCompatActivity {
    ...
    private void submitReport() {
        ...
        if (selectedCategory.isEmpty() || issueType.isEmpty() || location.isEmpty() || description.isEmpty()) {
            Toast.makeText(this, "Please fill in all required fields to submit the report.", Toast.LENGTH_LONG).show();
            return;
        }

        // 2. Validation for Media
        if (selectedMediaUri == null) {
            Toast.makeText(this, "Please attach a supporting photo or video.", Toast.LENGTH_LONG).show();
            return;
        }

        // --- Data Submission Placeholder ---

        String submissionSummary = "Report successfully filed!\n" +
                "Category: " + selectedCategory + "\n" +
                "Location: " + location + "\n" +
                "Media URI: " + selectedMediaUri.getLastPathSegment();

        // Display a confirmation message
        Toast.makeText(this, submissionSummary, Toast.LENGTH_LONG).show();

        // In a real application, you would now upload the media and save the report details.
    }
}