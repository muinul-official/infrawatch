...
public class MainActivity extends AppCompatActivity implements OnMapReadyCallback {
        ...
        bottomSheetBehavior.addBottomSheetCallback(new BottomSheetBehavior.BottomSheetCallback() {
            ...
            public void onStateChanged(View bottomSheet, int newState) {
                if (newState == BottomSheetBehavior.STATE_EXPANDED) {
                    fabToggleSheet.setImageResource(android.R.drawable.ic_menu_close_clear_cancel);
                } else if (newState == BottomSheetBehavior.STATE_COLLAPSED) {
                    fabToggleSheet.setImageResource(android.R.drawable.ic_menu_info_details);
                }
            }
                        ...
                        try {
                            ...
                            if (report != null) {
                                if (report.status != null && (report.status.equalsIgnoreCase("Completed")
                                        || report.status.equalsIgnoreCase("Closed"))) {
                                    continue;
                                }
                                reports.add(report);
                            }
    ...
    private void updateRecentActivities(List<RepairReport> allReports) {
        List<RepairReport> recent = allReports.size() > 3 ?
                allReports.subList(0, 3) : new ArrayList<>(allReports);
